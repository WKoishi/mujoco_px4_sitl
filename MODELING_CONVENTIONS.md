...
### What the first CAD export gave, and what was measured since

The export is a whole-vehicle SolidWorks export of the coaxial X8 plus a 5-DoF
arm, in the private repo; the URDF, its STLs, the filled sidecar, the generated
MJCF and the generated PX4 airframe all stay there, since the airframe's
`CA_ROTOR*_PX` / `_PY` block *is* the hub geometry. Naming already matched §3.3,
and the STLs are in metres. The conversion passes every self-check: mass and CoM
match the CAD figures, the 8 rotor sites are contiguous and on `base_link`, the
coaxial pairs' spins oppose, and the origin sits on the rotor symmetry centre to
sub-mm (§3.1). **The figures stay with the sidecar**; `urdf_to_mjcf.py
--check-only` prints them.

What the export did **not** carry:

- **Arm joint limits: every one is `lower=upper=0`**, which MuJoCo reads as
  *unlimited* (§6). The sidecar's ranges are not a placeholder for them: they are
  the mechanical limits, read off each joint's structure. Still unverified: the
  zero position and sign convention, since `arm_cmd.values[i]` is an absolute
  angle.
- **Spin directions and index→ESC mapping.** The sidecar assumes §4's PX4 order.
  Flight shows it consistent with the model (`IMPLEMENTATION_PLAN.md` phase 5);
  it still needs confirming against the wiring, where a mismatch presents as yaw
  drift.

**The motor fit** (below): least squares through the origin on the
manufacturer's table, `c_t` at R² = 0.9997 with the worst point 0.9 % off full
...
a disc, only at the front rotors, on both decks, and only with the last link
(figures in the sidecar). The limits cannot be the guard. Decided 2026-09-25: in
simulation an intrusion is **detected on the reached pose and reported, never
blocked** (§2.3, `README.md`); blocking is a real-vehicle safety layer for the
arm driver or the companion computer, outside this repo.
